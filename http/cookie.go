...
// parseCookie parses a cookie from a cookie header.
// For example:
//
//  1. Raw cookie:
//     ```
//     mycookie=value; othercookie=othervalue
//     ```
//
//  2. Parse cookie:
//     ```
//     cookie := ParseCookie("mycookie=value; othercookie=othervalue")
//     ```
//
//  3. Parse cookie from raw request:
//     ```
//     cookie := ParseCookie("Cookie: mycookie=value; othercookie=othervalue")
//     ```
func parseCookie(raw string) *Cookie {
	// TODO: implement cookie.readCookie(header,filter)?
...
func readSetCookies(h http.Header) []*Cookie {
	...
	// TODO: Make Header type [][]string.
	// Because it's not type [][]string, range header instead of index.
...
func (c *Cookie) String() string {
	...
	var buf [len(timeformat.TimeFormat)]byte
	if validCookieExpires(c.Expires) {
		b.WriteString("; Expires=")
		b.Write(c.Expires.UTC().AppendFormat(buf[:0], timeformat.TimeFormat))
	}
	if c.MaxAge > 0 {
...
func isCookieNameValid(raw string) bool {
	...
	return strings.IndexFunc(raw, token.IsNotToken) < 0
}
