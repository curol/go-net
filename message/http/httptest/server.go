...
func (s *Server) readCertFiles(certPath string, keyPath string) ([]byte, []byte) {
	// Read
...
func (s *Server) parseCert(certPEM, keyPEM []byte) *tls.Certificate {
	// Parse
...
func (s *Server) newTestCertificate(certPath string, keyPath string) *tls.Certificate {
	certPEM, keyPEM := s.readCertFiles(certPath, keyPath)
	cert := s.parseCert(certPEM, keyPEM)
	return cert
}
...
func (s *Server) StartTLS() {
	...
	// X509KeyPair parses a public/private key pair from
	// a pair of PEM encoded data.
	//
	// On successful return, Certificate.Leaf will be nil
	// because the parsed form of the certificate is not retained.
	// cert, err := tls.X509KeyPair(testcert.LocalhostCert, testcert.LocalhostKey)
	cert := s.newTestCertificate("mock/cert.pem", "mock/key.pem")
	if cert == nil {
		panic(fmt.Sprintf("httptest: NewTLSServer: %v", "cert can't be nil"))
	}
	...
	// Parse certificate to verify it parses correctly.
	parsedCert, err := x509.ParseCertificate(s.TLS.Certificates[0].Certificate[0])
	if err != nil {
		panic(fmt.Sprintf("httptest: NewTLSServer: %v", err))
	}
	s.certificate = parsedCert
